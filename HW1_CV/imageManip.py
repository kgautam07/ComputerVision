...
def mix_images(image1, image2, channel1, channel2):
    ...
    out = None
    ### YOUR CODE HERE
    firstImage = rgb_exclusion(image1, "R")
    secondImage = rgb_exclusion(image2, "G")
    height1, width1, _ = firstImage.shape
    height2, width2, _ = secondImage.shape

    leftFirstImage = firstImage[:, :width1//2, :]
    rightSecondImage = secondImage[:, width2//2:, :]


    out = np.concatenate((leftFirstImage, rightSecondImage), axis = 1)
...
def mix_quadrants(image):
    ...
    topLeft = image[:horizontal, :vertical, :]
    topRight = image[:horizontal, vertical:, :]
    bottomLeft = image[horizontal:, :vertical, :]
    bottomRight = image[horizontal:, vertical:, :]

    topLeft = rgb_exclusion(topLeft, "R")
    topRight = dim_image(topRight)
    bottomLeft = np.power(bottomLeft, 0.5)
    bottomRight = rgb_exclusion(bottomRight,"R")

    firstHalf = np.concatenate((topLeft,bottomLeft), axis = 0)
    secondHalf=np.concatenate((topRight,bottomRight),axis = 0)
    out = np.concatenate((firstHalf, secondHalf), axis = 1)
