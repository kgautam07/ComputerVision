...
def iterative_lucas_kanade(img1, img2, keypoints,
    ...
    for y, x, gy, gx in np.hstack((keypoints, g)):
        ...
        ### YOUR CODE HERE
        window_Ix = Ix[y1 - w: y1 + w + 1, x1 - w: x1 + w + 1]
        window_Iy = Iy[y1 - w: y1 + w + 1, x1 - w: x1 + w + 1]

        G = np.array([[np.sum(window_Ix ** 2), np.sum(window_Ix * window_Iy)],
        ...
        for k in range(num_iters):
            ...
            ### YOUR CODE HERE
            window_img1 = img1[y1 - w: y1 + w + 1, x1 - w: x1 + w + 1]
            window_img2 = img2[y1 - w: y1 + w + 1, x1 - w: x1 + w + 1]

            bk = np.array([np.sum(window_Ix * (window_img2 - window_img1)),
               np.sum(window_Iy * (window_img2 - window_img1))])
            vk = np.dot(inv_G, bk)
            ### END YOUR CODE
...
def pyramid_lucas_kanade(img1, img2, keypoints,
    ...
    for L in range(level, -1, -1):
        ...
        flow_vectors = iterative_lucas_kanade(img1_L, img2_L, scaled_keypoints,  window_size=window_size,                                  num_iters=num_iters, g=g)
        flow_vectors *= (scale ** L)
        g = flow_vectors 
        ### END YOUR CODE
...
def compute_error(patch1, patch2):
    ...
    error = 0
    ### YOUR CODE HERE
    patch1_norm = patch1 / 255.0
    patch2_norm = patch2 / 255.0

    # Calculate squared differences between patches
    squared_diff = (patch1_norm - patch2_norm) ** 2
