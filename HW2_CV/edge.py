...
def conv(image, kernel):
    ...
    for i in range(Hi):
        for j in range(Wi):
            temp = padded[i:i+Hk, j:j+Wk]
            out[i, j] = np.sum(temp * kernel)
...
def gaussian_kernel(size, sigma):
    ...
    ### YOUR CODE HERE
    k = size // 2  # Find the k of the kernel.
    firstPart = 1 / (2 * np.pi * sigma ** 2)
    ...
    for i in range(size):
        for j in range(size):
            ...
            kernel[i, j] = firstPart * np.exp(-((x**2 + y**2) / (2 * sigma**2)))
...
def link_edges(strong_edges, weak_edges):
    ...
    for i in range(H):
        for j in range(W):
            if strong_edges[i, j]:
                for ny, nx in get_neighbors(i, j, H, W):
                    if weak_edges[ny, nx]:
                        edges[ny, nx] = True
...
def hough_transform(img):
    ...
    for i in range(len(xs)):
        ...
        for j in range(num_thetas):
            rho = int(x * cos_t[j] + y * sin_t[j] + diag_len)
            accumulator[rho, j] += 1
